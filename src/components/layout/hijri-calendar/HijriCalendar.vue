...
export default {
    ...
    data() {
        return {
          ...
          monthNames: this.$props.monthNames || this.$primevue.config.locale.monthNames,
          monthNamesShort: this.$props.monthNamesShort || this.$primevue.config.locale.monthNamesShort,
        };
    },
    watch: {
        modelValue(newValue) {
    ...
    methods: {
        ...
        isDateBetween(start, end, dateMeta) {
            ...
            if (start && end) {
                let date = new HijriDate(dateMeta.year, dateMeta.month, dateMeta.day);
        ...
        getFirstDayOfMonthIndex(month, year) {
            let day = new HijriDate(year, month, 1);
        ...
        getDaysCountInMonth(month, year) {
            // console.log(`month: ${month+1}`); // dubugging
            let hijriNow = this.daylightSavingAdjust(new HijriDate(year, month, 1));

            // we use moment-hijri package to get the last date of the month because hijri-date package doesn't yet have methods for that. the closes workaround is to go to the forst day of the next month and subtract a day. But the subtractDay method is still in "to-do"

            // index 0 for first month. To get the real month from today, we add 1
            let dateString = `${hijriNow.getFullYear()}/${hijriNow.getMonth()+1}/1`
            let daysInMonth = moment(moment(dateString, 'iYYYY/iM/iD').endOf('iMonth')).iDate()
            // console.log(`days in month: ${daysInMonth}`); // dubugging
            return daysInMonth;
        },
        ...
        daylightSavingAdjust(date) {
            ...
            let hours = date.getHours() > 12 ? date.getHours() + 2 : 0
            date = new HijriDate(date.getFullYear(), date.getMonth(), date.getDay(), hours);
        ...
        isDayDisabled(day, month, year) {
            if (this.disabledDays) {
                let weekday = new HijriDate(year, month, day);
        ...
        selectDate(dateMeta) {
            ...
            if (this.minDate && toHijri(this.minDate) > toHijri(date)) {
                let gregDate = this.minDate
                date = toHijri(this.minDate);
                this.currentHour = gregDate.getHours();
                this.currentMinute = gregDate.getMinutes();
                this.currentSecond = gregDate.getSeconds();
            }

            if (this.maxDate && toHijri(this.maxDate) < toHijri(date)) {
                let gregDate = this.maxDate
                date = toHijri(this.maxDate);
                this.currentHour = gregDate.getHours();
                this.currentMinute = gregDate.getMinutes();
                this.currentSecond = gregDate.getSeconds();
            }
                    ...
                    if (literal) {
                        ...
                        switch (format.charAt(iFormat)) {
                            ...
                            case 'D':
                                ...
                                break;
                            case 'o':
                                output += formatNumber('o', Math.round((new HijriDate(date.getFullYear(), date.getMonth(), date.getDate()).getTime() - new HijriDate(date.getFullYear(), 0, 0).getTime()) / 86400000), 3);
                                break;
                            case 'm':
        ...
        updateModelTime() {
            ...
            value = value ? new HijriDate(value.getTime()) : new HijriDate();
        ...
        parseDate(value, format) {
            ...
            let iFormat,
                ...
                extra,
                iValue = 0,
                shortYearCutoff = typeof this.shortYearCutoff !== 'string' ? this.shortYearCutoff : (new HijriDate().getFullYear() % 100) + parseInt(this.shortYearCutoff, 10),
                year = -1,
                month = -1,
                day = -1,
                doy = -1,
                ...
                if (literal) {
                    ...
                    switch (format.charAt(iFormat)) {
                        ...
                        case 'y':
                            ...
                            break;
                        case '@':
                            date = new HijriDate(getNumber('@'));
                            year = date.getFullYear();
                            month = date.getMonth() + 1;
                            day = date.getDate();
                            break;
                        case '!':
                            date = new HijriDate((getNumber('!') - this.ticksTo1970) / 10000);
                            year = date.getFullYear();
                            month = date.getMonth() + 1;
                            day = date.getDate();
                            break;
            ...
            if (year === -1) {
                year = new HijriDate().getFullYear();
            } else if (year < 100) {
                year += new HijriDate().getFullYear() - (new HijriDate().getFullYear() % 100) + (year <= shortYearCutoff ? 0 : -100);
            }
            ...
            date = this.daylightSavingAdjust(new HijriDate(year, month - 1, day));
        ...
        getWeekNumber(date) {
            let checkDate = new HijriDate(date.getTime());
    ...
    computed: {
        ...
        months() {
            ...
            for (let i = 0; i < this.numberOfMonths; i++) {
                ...
                let year = this.currentYear ?? HijriDate.today().getFullYear();
                ...
                for (let i = 0; i < monthRows; i++) {
                    ...
                    if (this.showWeek) {
                        weekNumbers.push(this.getWeekNumber(new HijriDate(week[0].year, week[0].month, week[0].day)));
                    }
        ...
        monthPickerValues() {
            ...
            for (let i = 0; i <= 11; i++) {
                monthPickerValues.push(this.monthNamesShort[i]);
            }
